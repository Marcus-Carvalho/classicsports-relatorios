...
import sys
import io
# Redireciona stdout/stderr para UTF-8 apenas se nao estiver sendo redirecionado para arquivo
try:
...
def copiar_perfil():
    ...
    # Arquivos de sessao/login sao os unicos necessarios para autenticacao
    ARQUIVOS_ESSENCIAIS = [
    ...
    # Copia arquivos essenciais da raiz do perfil
...
def fazer_procv_completo():
    ...
    if ci_status and ci_preco_por and ci_preco_min:
        ...
        ws.column_dimensions[get_column_letter(col_insert)].width = 8
        col_map = {ws.cell(1,c).value: c for c in range(1, ws.max_column+1)}
        FILLS["%"] = fill_pct
        ...
        async with async_playwright() as p:
            ...
            async def _baixar_relatorio(destino, timeout_min=40, antes=None):
                ...
                while _t < timeout_min * 60:
                    ...
                    # ainda baixando? existe algum .crdownload na pasta
